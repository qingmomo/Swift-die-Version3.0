...
//    MARK: 创建tabBar
    func createTabBar() {
        ...
        for i in 0..<3 {
        let tabBarBtn = TabBarButton(frame: CGRect(x: kWidth/3*CGFloat(i), y: 0, width: kWidth/3, height: 49))
            if i==0{
...
//            tabBarBtn.addTarget(self, action: "btnAction:" , forControlEvents: .TouchUpInside)
            tabBarBtn.addTarget(self, action: #selector(BaseTabBarController.btnAction(_:)), for: .touchUpInside)
            tabBarBtn.tag = 100+i
    ...
    func btnAction(_ btn : TabBarButton) {
        self.selectedIndex = btn.tag - 100
        
        for tabBarBtn in self.tabBar.subviews{
            if (tabBarBtn is TabBarButton) {
